...
model_save_dir = '/home/xiao/git_projects/auto_AM_researcher/Xiao/MeltPool_MLWorkFlow/trained_models/'
...
final_df.to_csv(cwd+'processed_data_for_training_3264.csv', index=False)
...
model = Sequential([
    Dense(32, activation='relu', input_shape=(X_train_std.shape[1],),kernel_regularizer=regularizers.l2(0.1)),
    Dense(64, activation='relu', input_shape=(X_train_std.shape[1],),kernel_regularizer=regularizers.l2(0.1)),
    Dense(y_train_pca.shape[1], activation='linear')
])
...
history = model.fit(X_train_std, y_train_pca, epochs=1000000, validation_split=0.2, 
                    callbacks=[early_stopping], verbose=1)

y_pred_pca = model.predict(X_test_std)
y_pred = pca.inverse_transform(y_pred_pca)

# model.save(model_save_dir + 'para2geom.h5')
# # Save the PCA transformer
# joblib.dump(pca, model_save_dir + 'pca_transformer.pkl')
# # Save standard scaler
# joblib.dump(sc, model_save_dir + 'sc.bin')
...
plt.savefig(cwd+'trained_models/MLP_mp.svg', format='svg', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(cwd+'trained_models/MLP_Parity.svg', dpi=300)
plt.show()
